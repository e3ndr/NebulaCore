...
public class AbstractPlaceholders {
    ...
    public String formatNative(OfflinePlayer player, String text) {
        ...
        return ret.replace("\\n", "\n").replace("%online%", String.valueOf(online)).replace("%online_s%", online_s).replace("%online_isAre%", online_isAre).replace("%player_username%", player_username).replace("%player_displayname%", (player_displayname == null) ? player_username : player_displayname).replace("%player_nickname%", (player_nickname == null) ? "" : player_nickname).replace("%player_name%", (player_nickname == null) ? ((player_displayname == null) ? player_username : player_displayname) : player_nickname).replace("%player_chatcolor%", (player_chatcolor == null) ? "" : player_chatcolor).replace("%player_chattag%", (player_chattag == null) ? "" : player_chattag).replace("%player_world%", player_world).replace("%player_ping%", player_ping).replace("%player_balance%",
                player_balance);
    }
}
