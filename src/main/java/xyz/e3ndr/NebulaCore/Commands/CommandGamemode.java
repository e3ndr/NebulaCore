	...
	public void onCommand(CommandSender executor, String alias, String[] args, boolean isConsole) {
		if (!isConsole) {
			...
			switch (alias.toLowerCase()) {
				case "gms": this.gamemode("surVival", executor, player); return;
				case "survival": this.gamemode("0", executor, player); return;
				
				case "gmc": this.gamemode("1", executor, player); return;
				case "creative": this.gamemode("CreatIvE", executor, player); return;
				
				case "gma": this.gamemode("a", executor, player); return;
				case "adventure": this.gamemode("2", executor, player); return;
				
				case "gmsp": this.gamemode("sPEctATor", executor, player); return;
				case "spectator": this.gamemode("3", executor, player); return;
