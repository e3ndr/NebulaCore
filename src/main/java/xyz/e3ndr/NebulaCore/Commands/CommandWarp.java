				...
				if (!player.hasPermission("Nebula.warps.list")) {
					...
					if (warps.size() != 0) {
						for (Warp warp : warps) {
							...
							String w = NebulaCore.getLang("warp.individual", player.getUniqueId(), false)
									.replace("%warp%", warp.name)
									.replace("%warp_prefix%", player.hasPermission(warp.perm) ? NebulaCore.getLang("general.prefix.good", player.getUniqueId(), false) : NebulaCore.getLang("general.prefix.bad", player.getUniqueId(), false));
