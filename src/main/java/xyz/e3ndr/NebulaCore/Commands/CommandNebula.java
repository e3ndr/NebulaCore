...
import xyz.e3ndr.NebulaCore.BannerProducer;
...
public class CommandNebula extends BaseCommand {
	...
	public void onCommand(CommandSender executor, String alias, String[] args, boolean isConsole) {
		if ((args.length > 0) && args[0].equalsIgnoreCase("author")) {
			...
			if ((args.length == 0) || args[0].equalsIgnoreCase("info")) {
				...
				NebulaCore.instance.reload();
				executor.sendMessage(ChatColor.translateAlternateColorCodes('&', "&aReloaded all Nebula modules, configs, and integration."));
			// } else if (executor.hasPermission("Nebula.nebula.gui") && args[0].equalsIgnoreCase("gui")) {
				
			} else if (isConsole && args[0].equalsIgnoreCase("banner")) {
				executor.sendMessage(BannerProducer.banner());
			} else {
				this.info(executor);
			}
		} else {
	...
	private void modules(CommandSender executor) {
		...
		for (AbstractModule module : NebulaCore.instance.modules) {
			sb.append("\n");
	...
	private void info(CommandSender executor) {
		executor.sendMessage(ChatColor.translateAlternateColorCodes('&', new StringBuilder().append("&5Running &dNebula &5version &d").append(NebulaCore.instance.getDescription().getVersion()).append("&5.").toString()));
	}
	...
	public ArrayList<String> onTabComplete(CommandSender executor, String alias, String[] args, boolean isConsole) {
		...
		if (executor.hasPermission("Nebula.nebula") && (args.length == 1)) {
			...
			// if (!isConsole && executor.hasPermission("Nebula.nebula.gui")) ret.add("gui");
