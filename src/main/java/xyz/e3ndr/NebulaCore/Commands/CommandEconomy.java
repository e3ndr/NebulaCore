	...
	public void onCommand(CommandSender executor, String alias, String[] args, boolean isConsole) {
		if (alias.equalsIgnoreCase("bal")) {
			...
			if (args.length > 0) {
				...
				switch (args[0]) {
					case "show": this.balance(executor, (args.length > 1) ? args[1] : null, isConsole); return;
					case "balance": this.balance(executor, (args.length > 1) ? args[1] : null, isConsole); return;
					case "bal": this.balance(executor, (args.length > 1) ? args[1] : null, isConsole); return;
					
					case "set": this.set(executor, args); return;
					case "give": this.add(executor, args); return;
					case "add": this.add(executor, args); return;
					case "take": this.take(executor, args); return;
					case "remove": this.take(executor, args); return;
		...
		if (alias.equalsIgnoreCase("bal")) {
			...
			if (args.length == 1) {
				...
				if (executor.hasPermission("Nebula.balance")) {
					...
					executor.hasPermission("Nebula.economy.set") ||
					executor.hasPermission("Nebula.economy.add") ||
					executor.hasPermission("Nebula.balance.others"))) {
				ret.addAll(Util.getPlayerNames());
			}
		}
